...
% dynamics of an airframe. The states are the Earth coordinates
% $(X_e,Z_e)$, the body coordinates $(u,w)$, the pitch angle $\theta$, and
% the pitch rate $q = \dot\theta$. The following figure summarizes the
% relationship between the inertial and body frames, the flight path angle
% $\gamma$, the incidence angle $\alpha$, and the pitch angle $\theta$.
%
% <<../gsautopilot1.png>>
%
% We use a classic three-loop autopilot structure to control the flight
% path angle $\gamma$. This autopilot adjusts the flight path by delivering
% adequate bursts of normal acceleration $a_z$ (acceleration along $w$). In
% turn, normal acceleration is produced by adjusting the elevator
% deflection $\delta$ to cause pitching and vary the amount of lift. The
% autopilot uses Proportional-Integral (PI) control in the pitch rate loop
% $q$ and proportional control in the $a_z$ and $\gamma$ loops. The
% closed-loop system (airframe and autopilot) are modeled in Simulink.
open_system('rct_airframeGS')

%% Autopilot Gain Scheduling
% The airframe dynamics are nonlinear and the aerodynamic forces and
% moments depend on speed $V$ and incidence $\alpha$. To obtain suitable
% performance throughout the $(\alpha,V)$ flight envelope, the autopilot
% gains must be adjusted as a function of $\alpha$ and $V$ to compensate
% for changes in plant dynamics. This adjustment process is called "gain
% scheduling" and $\alpha,V$ are called the scheduling variables. In the
% Simulink model, gain schedules are implemented as look-up tables driven
% by measurements of $\alpha$ and $V$.
%
% Gain scheduling is a linear technique for controlling nonlinear or
% time-varying plants. The idea is to compute linear approximations of the
% plant at various operating conditions, tune the controller gains at each
% operating condition, and swap gains as a function of operating condition
% during operation. Conventional gain scheduling involves the following
% three major steps.
%
% # Trim and linearize the plant at each operating condition
% # Tune the controller gains for the linearized dynamics at each operating
% condition
% # Reconcile the gain values to provide smooth transition between
%   operating conditions.
%
% In this example, you combine steps 2 and 3 by parameterizing the
% autopilot gains as first-order polynomials in $\alpha,V$ and directly
% tuning the polynomial coefficients for the entire flight envelope. This
% approach eliminates step 3 and guarantees smooth gain variations as a
% function of $\alpha$ and $V$. Moreover, the gain schedule coefficients
% can be automatically tuned with |systune|.

%% Trimming and Linearization
% Assume that the incidence $\alpha$ varies between -20 and 20 degrees and
% that the speed $V$ varies between 700 and 1400 m/s. When neglecting
% gravity, the airframe dynamics are symmetric in $\alpha$. Therefore,
% consider only positive values of $\alpha$. Use a 5-by-9 grid of linearly
% spaced $(\alpha,V)$ pairs to cover the flight envelope.
...
%%
% Use |operspec| to specify the trim condition, use |findop| to compute the
% trim values of $\delta$ and $q$, and linearize the airframe dynamics for
% the resulting operating points. For details, see
% <docid:slcontrol_ug#mw_654b6e1e-464a-4316-9cda-a65f0e60f324 Trimming and
% Linearizing an Airframe>. Repeat these steps for the 45 flight conditions
% $(\alpha,V)$.
...
%%
% This process produces a 5-by-9 array of linearized plant models at the 45  
...
% "scheduled" (adjusted) as a function of $\alpha$ and $V$. Practically,
% this means tuning 88 values in each of the corresponding four look-up
% tables. Rather than tuning each table entry separately, parameterize the
% gains as a two-dimensional gain surfaces, for example, surfaces with a
% simple multi-linear dependence on $\alpha$ and $V$:
%
% $$ K(\alpha,V) = K_0 + K_1 \alpha + K_2 V + K_3 \alpha V $$.
%
% This cuts the number of variables from 88 down to 4 for each lookup
% table. Use the |tunableSurface| object to parameterize each gain surface.
% Note that:
%
...
%
% Each surface is initialized to a constant gain using the tuning results
% for $\alpha$ = 10 deg and $V$ = 1050 m/s (mid-range design).
...
%%
% Next create an |slTuner| interface for tuning the gain surfaces. Use
% block substitution to replace the nonlinear plant model by the linearized
% models over the tuning grid. Use |setBlockParam| to associate the tunable
% gain surfaces |Kp|, |Ki|, |Ka|, |Kg| with the Interpolation blocks of the
% same name.
...
%
% * $\gamma$ loop: Track the setpoint with a 1 second response time, less
% than 2%
...
%   (to track acceleration demands) and past 10 rad/s (to be insensitive to
%   measurement noise). The disturbance is injected at the |az_ref|
%   location.
...
%%
% Simulate the autopilot performance for a maneuver that takes the 
...
%%
% Tracking of the flight path angle profile remains good throughout the
% maneuver. Note that the variations in incidence $\alpha$ and speed $V$
% cover most of the flight envelope considered here ([-20,20] degrees for
% $\alpha$ and [700,1400] for $V$). And while the autopilot was tuned for a
% nominal altitude of 3000 m, it fares well for altitude changing from
% 1,000 to 10,000 m.
%
% The nonlinear simulation results confirm that the gain-scheduled
% autopilot delivers consistently high performance throughout the flight
% envelope. The "gain surface tuning" procedure provides simple explicit
% formulas for the gain dependence on the scheduling variables. Instead of
% using look-up tables, you can use these formulas directly for an more
